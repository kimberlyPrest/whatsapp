...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const EVOLUTION_INSTANCE = Deno.env.get('EVOLUTION_INSTANCE') ?? 'org-prestes'

    if (!EVOLUTION_URL || !EVOLUTION_KEY) {
      throw new Error('Configuração incompleta: EVOLUTION_API_URL e EVOLUTION_API_KEY são obrigatórios')
    }
    ...
    if (suggestion_id) {
      ...
      if (sugg?.sent_text) {
        return new Response(
          JSON.stringify({ success: true, already_sent: true, phone_number: base }),
    ...
    const qualityRating = similarity > 0.98 ? 5 : similarity > 0.85 ? 4 : similarity > 0.6 ? 3 : 2
    ...
    if (!evolutionRes.ok) {
      ...
      if (suggestion_id) {
        await supabase.from('suggestions').update({ status: 'failed_to_send' }).eq('id', suggestion_id)
      }
    ...
    await supabase
      .from('conversations')
      .update({ last_message_text: trimmedText, last_message_at: approvedAt, last_sender: 'me' })
    ...
    if (suggestion_id) {
      await supabase
        ...
        .update({
          ...
          edit_diff: JSON.stringify({ similarity: Math.round(similarity * 100) / 100 }),
      ...
      if (wasEdited) {
        await supabase.from('training_feedback').insert({
          ...
          evidence: JSON.stringify({ suggestion_id, phone_number: base, similarity: Math.round(similarity * 100) / 100 }),
          status: 'pending',
        })
      }
    }

    console.log(`✅ Mensagem enviada | phone: ${base} | editada: ${wasEdited} | quality: ${qualityRating}`)
