...
const TLDV_HEADERS = { 'x-api-key': TLDV_KEY, 'Content-Type': 'application/json' }
...
const GENERIC_DOMAINS = new Set(['gmail.com', 'hotmail.com', 'outlook.com', 'yahoo.com', 'icloud.com', 'terra.com.br', 'uol.com.br', 'bol.com.br', 'live.com'])

async function main() {
  console.log('📦 Carregando dados...')
  const profilesRes = await fetch(`${SUPABASE_URL}/rest/v1/client_profiles?select=id,contact_name,email&propriedade=eq.true`, { headers: SUPABASE_HEADERS })
  const profiles: any[] = await profilesRes.json()

  const emailMap = new Map(); const nameMap = new Map(); const domainMap = new Map(); const firstNameMap = new Map()
  ...
  for (const p of profiles) {
    if (p.email) {
      ...
      if (dom && !GENERIC_DOMAINS.has(dom)) domainMap.set(dom, domainMap.has(dom) ? 'multiple' : p.id)
    }
    if (p.contact_name) {
      const normName = normalize(p.contact_name)
      nameMap.set(normName, p.id)
      const first = normName.split(' ')[0]
      if (first.length > 3) firstNameMap.set(first, firstNameMap.has(first) ? 'multiple' : p.id)
    }
  }
  ...
  for (const m of allMeetings) {
    ...
    for (const inv of invitees) {
      ...
      if (emailMap.has(e)) { foundId = emailMap.get(e); break }
    }
    // 2. Domínio corp
    if (!foundId) {
      for (const inv of invitees) {
        const d = (inv.email ?? '').split('@')[1]?.toLowerCase()
        if (d && domainMap.get(d) && domainMap.get(d) !== 'multiple') { foundId = domainMap.get(d); break }
      }
    }
    // 3. Nome no título
    if (!foundId) {
      // Tenta nome completo
      for (const [n, id] of nameMap) { if (n.length > 5 && title.includes(n)) { foundId = id; break } }
      // Se título tem "melisa" e só temos uma Melisa
      if (!foundId) {
        for (const [f, id] of firstNameMap) {
          if (id !== 'multiple' && (title.endsWith(` ${f}`) || title.includes(` ${f} `) || title.includes(` <> ${f}`))) { foundId = id; break }
        }
      }
    }
  ...
  const mcRes = await fetch(`${SUPABASE_URL}/rest/v1/meus_clientes?select=*`, { headers: SUPABASE_HEADERS })
  ...
  for (const mc of meusClientes) {
    ...
    const latest = meetings.sort((a, b) => new Date(b.happenedAt).getTime() - new Date(a.happenedAt).getTime())[0]
    ...
    if (Object.keys(updates).length > 0) {
      await fetch(`${SUPABASE_URL}/rest/v1/meus_clientes?id=eq.${mc.id}`, { method: 'PATCH', headers: SUPABASE_HEADERS, body: JSON.stringify(updates) })
      count++
    }
  }

  console.log(`✅ Sincronização concluída: ${count} clientes atualizados.`)
  console.log(`🔍 Total de ${profileToMeetings.size} clientes vinculados a reuniões.`)
}
