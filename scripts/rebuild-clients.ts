...
async function fetchCSV(sheetId: string): Promise<string[][]> {
    ...
    if (!res.ok) throw new Error(`Erro ao buscar planilha ${sheetId}: ${res.status}`)
...
async function supabasePost(path: string, body: any, prefer = 'resolution=merge-duplicates,return=minimal') {
...
async function main() {
    ...
    const existingProfiles: { phone_number: string; id: string }[] = await supabaseGet('client_profiles?select=phone_number,id')
    ...
    const toInsert: { phone_number: string; propriedade: boolean; created_at: string }[] = []
    for (const c of conversations) {
        if (!findInDb(c.phone_number)) {
            toInsert.push({ phone_number: c.phone_number, propriedade: false, created_at: now })
        ...
        for (let i = 0; i < toInsert.length; i += BATCH) {
            ...
            try {
                ...
                for (const item of batch) {
                    try { await supabasePost('client_profiles?on_conflict=phone_number', item) } catch { }
                }
            }
        }
    }
    ...
    // Recarrega profiles
    const allProfiles: { phone_number: string; id: string }[] = await supabaseGet('client_profiles?select=phone_number,id')
    ...
    let s1Updated = 0, s1Skipped = 0

    for (const cols of rows1) {
        const rawPhone = cols[s1idx.tel] ?? ''
        const phone = normalizePhone(rawPhone)
        if (!phone) { s1Skipped++; continue }

        const dbPhone = findPhone(phone)
        if (!dbPhone) { s1Skipped++; continue } // Não existe no banco, ignora
        ...
        // Valida email
        const validEmail = email && email.includes('@') && !email.toLowerCase().includes('não encontrado') ? email : null
        ...
        try {
            await supabasePatch(`client_profiles?phone_number=eq.${encodeURIComponent(dbPhone)}`, updates)
            s1Updated++
        } catch {
            // silently skip
        }
    ...
    console.log(`   ⏭  ${s1Skipped} linhas ignoradas (sem telefone ou não existe no banco).`)
    ...
    const s2idx = {
        ...
        call1: h2l.findIndex(h => h.includes('1') && h.includes('call') || h.includes('1ª call') || h.includes('data 1')),
        call2: h2l.findIndex(h => h.includes('2') && h.includes('call') || h.includes('2ª call') || h.includes('data 2')),
        csat1: h2l.findIndex(h => h.includes('avalia') || h.includes('satisfaç') || h.includes('nota') || (h.includes('csat') && !h.includes('coment'))),
        csatCom1: h2l.findIndex(h => h.includes('comentário') && (h.includes('primeir') || h.includes('1'))),
        csatCom2: h2l.findIndex(h => h.includes('comentário') && (h.includes('segund') || h.includes('2'))),
    }
    ...
    for (const cols of rows2) {
        ...
        const csatC1 = s2idx.csatCom1 >= 0 ? (cols[s2idx.csatCom1]?.trim() || null) : null
        const csatC2 = s2idx.csatCom2 >= 0 ? (cols[s2idx.csatCom2]?.trim() || null) : null
        ...
        if (dbPhone) {
            ...
            try {
                await supabasePost('client_profiles?on_conflict=phone_number', newProfile)
                // Tenta também com a variante
                const alt = brAlternate(phone)
                if (alt) {
                    try {
                        await supabasePost('client_profiles?on_conflict=phone_number', { ...newProfile, phone_number: alt })
                    } catch { }
                }
    ...
    await supabasePatch('client_profiles?propriedade=eq.true', { propriedade: false, updated_at: now })
    ...
    for (const phone of sheet2Phones) {
        try {
            await supabasePatch(`client_profiles?phone_number=eq.${encodeURIComponent(phone)}`, { propriedade: true })
        } catch { }
    }
    ...
    const finalProfiles: { phone_number: string; id: string }[] = await supabaseGet('client_profiles?select=phone_number,id&propriedade=eq.true')
    const finalPhoneToId = new Map(finalProfiles.map(p => [p.phone_number, p.id]))
    ...
    console.log(`   Etapa 3: ${s2Updated} atualizados + ${s2Created} criados (Planilha 2)`)
    console.log(`   Etapa 4: ${sheet2Phones.size} com propriedade=true, ${mcInserted} em meus_clientes`)
