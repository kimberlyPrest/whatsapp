...
export default function AIAgent() {
    ...
    async function loadData() {
        ...
        try {
            ...
            toast({ variant: 'destructive', title: 'Erro', description: 'Erro ao carregar configurações' })
        } finally {
            setLoading(false)
        }
    }
    ...
    async function loadFeedbacks() {
        try {
            ...
            toast({ variant: 'destructive', title: 'Erro', description: 'Erro ao carregar feedbacks' })
        }
    }
    ...
    const handleSaveRule = async () => {
        ...
        const payload = {
            ...formData,
            trigger_patterns: formData.trigger_patterns.split(',').map(s => s.trim()).filter(Boolean),
        }
    ...
    const handleDeleteRule = async (id: string) => {
        ...
        try {
            ...
            toast({ variant: 'destructive', title: 'Erro', description: 'Erro ao excluir regra' })
        }
    }

    const handleToggleRuleStatus = async (id: string, currentStatus: boolean) => {
        try {
            await whatsappService.updateAutonomousRule(id, { is_active: !currentStatus })
        ...
        try {
            ...
            if (status === 'approved') {
                if (feedback.feedback_type === 'new_rule') {
                    ...
                    await whatsappService.updateAIPrompt(prompts[0].id, feedback.suggested_value)
                    toast({ title: 'Prompt Atualizado', description: 'O comando da IA foi atualizado' })
                }
            }
        ...
        <div className="p-4 md:p-8 space-y-6 bg-[#F8FAFC] min-h-screen">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="flex flex-col gap-1">
                    <h1 className="text-3xl font-bold tracking-tight text-[#111B21]">Gestão do Agente IA</h1>
                    <p className="text-gray-500">Configure automações e treine o modelo com base no histórico.</p>
                </div>
                <Button
            ...
            <Tabs defaultValue="rules" className="w-full">
                <TabsList className="bg-white border p-1 rounded-xl shadow-sm mb-6">
                    <TabsTrigger value="rules" className="data-[state=active]:bg-[#25D366]/10 data-[state=active]:text-[#25D366]">
                        <Zap className="w-4 h-4 mr-2" />
                        Regras Autônomas
                    </TabsTrigger>
                    <TabsTrigger value="training" className="data-[state=active]:bg-[#25D366]/10 data-[state=active]:text-[#25D366]">
                        <BrainCircuit className="w-4 h-4 mr-2" />
                        Treinamento
                    </TabsTrigger>
                    <TabsTrigger value="prompts" className="data-[state=active]:bg-[#25D366]/10 data-[state=active]:text-[#25D366]">
                                            ...
                                            <TableCell>
                                                <div className="flex flex-wrap gap-1">
                                                    {rule.trigger_patterns?.map((t: string) => (
                                                        <Badge key={t} variant="outline" className="text-[10px] lowercase">
                                                            {t}
                                                        </Badge>
                                                    ))}
                                                </div>
                    ...
                    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                        {feedbacks.length > 0 ? feedbacks.map((f) => (
                            ...
                            <div className="col-span-full py-20 text-center text-gray-500">
                                Nenhum feedback {feedbackStatus === 'pending' ? 'pendente' : ''} no momento.
                            </div>
                        )}
                    </div>
                </TabsContent>
                ...
                <TabsContent value="prompts">
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>Defina como a IA deve se comportar quando não houver uma regra específica.</CardDescription>
                        </CardHeader>
                        <CardContent>
                            {prompts.map((p) => (
            ...
            <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
                <DialogContent className="max-w-2xl text-[#111B21]">
                    ...
                    <div className="grid gap-4 py-4">
                        ...
                        <div className="space-y-2">
                            <Label htmlFor="triggers">Palavras-chave (gatilhos separar por vírgula)</Label>
                            <Input
                                id="triggers"
                                value={formData.trigger_patterns}
                                onChange={(e) => setFormData({ ...formData, trigger_patterns: e.target.value })}
                        ...
                        <div className="flex items-center justify-between border p-4 rounded-lg bg-gray-50">
                            <div className="space-y-0.5">
                                ...
                                <p className="text-xs text-muted-foreground">Enviar sem pedir aprovação do operador.</p>
                            </div>
                            <Switch
                                checked={formData.auto_send}
                                onCheckedChange={(val) => setFormData({ ...formData, auto_send: val })}
                            />
                        </div>
                        ...
                        <div className="flex items-center justify-between border p-4 rounded-lg bg-gray-50">
                            <div className="space-y-0.5">
                                ...
                                <p className="text-xs text-muted-foreground">Marcar conversa como resolvida após o envio.</p>
                            </div>
                            <Switch
                                checked={formData.should_close}
                                onCheckedChange={(val) => setFormData({ ...formData, should_close: val })}
                            />
                        </div>
                    </div>
                    ...
                    <DialogFooter>
                        ...
                        <Button onClick={handleSaveRule} className="bg-[#25D366] hover:bg-[#1fb355] text-white">
                            Salvar Configuração
                        </Button>
                    </DialogFooter>
                </DialogContent>
